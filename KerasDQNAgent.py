...
class KerasDQNAgent(object):
    ...
    def configure(self, observation_space_shape, nb_actions):
        ...
        self.dqn = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, nb_steps_warmup=10,
               target_model_update=1e-2, policy=policy)
        self.dqn.compile(Adam(lr=1e-3), metrics=['mae'])
        
    def train(self, env, nb_steps, visualize, verbosity):
        self.dqn.fit(env, nb_steps=nb_steps, visualize=visualize, verbose=verbosity)
    
    def test(self, env, nb_episodes, visualize):
        self.dqn.test(env, nb_episodes=nb_episodes, visualize=visualize)
